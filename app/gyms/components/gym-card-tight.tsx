...
export default function GymCardTight({
  gym,
  selected = false,
  onSelect,
}: {
  ...
  return (
    <div
      ...
      } mb-2 card rounded-none cursor-pointer shadow-xl transition-colors`}
      onClick={handleCardClick}
    >
      <div className="card-body py-2 px-4">
        <h2 className="card-title mb-1">{gym.name}</h2>
        <p className="leading-tight">{gym.address.line1}</p>
        <p className="leading-tight mb-1">{gym.address.line2 ?? "\u00A0"}</p>
        ...
        {selected ? (
          <div className="card-actions w-full md:w-96 lg:w-full block mt-2">
